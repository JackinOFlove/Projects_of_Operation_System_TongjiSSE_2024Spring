...
def Elevator_Update(elevator_num):
    ...
    if not Elevator_Alert[elevator_num - 1]:
        ...
        UpdateLCDnumber.display(Elevator_Floor[elevator_num - 1])
        #更新电梯的标签信息
...
def Set_Elevatorlabel(elevator_num):
    ...
    #电梯上行状态和下行状态的箭头标签
        ...
        label.setText(f"{'↑' if Elevator_State[elevator_num - 1] == 1 else '↓'}")  
...
def Elevator_OpenDoor(elevator_num):
    ...
    if Elevator_State[elevator_num - 1] == 0 and Elevator_Alert[elevator_num - 1] == False:
    ...
    #如果电梯不处于停留状态有两种情况
        ...
        #如果此时电梯报警：不开门
        if Elevator_Alert[elevator_num - 1] == True:
            #代码的健壮性，并且文本输出电梯信息
            if updateInfo is not None:
